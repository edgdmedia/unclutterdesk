  ...
  return (
    <div className="min-h-screen bg-[#FCFDFE] text-[#0F172A] font-outfit flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-[560px] space-y-6 text-center">
        ...
        <div>
          <h1 className="text-[32px] font-extrabold tracking-[-0.035em] text-[#0F172A]">{booking.manualPayment ? 'Your time is held' : 'Your session is booked'}</h1>
          <p className="text-[15px] text-[#475569] font-medium max-w-[460px] mx-auto mt-2 leading-relaxed">
            {booking.manualPayment
              ? 'Send the transfer below to confirm it. We have emailed you these details too.'
              : 'Now create your client account so you can sign in, manage sessions, and join telehealth rooms securely.'}
          </p>
        </div>
        {booking.manualPayment ? (
          <TransferDetails payment={booking.manualPayment} bookingId={booking.bookingId} email={state.email} color={primaryColor} />
        ) : null}
        <div className="rounded-[24px] bg-white border border-[#E2E8F0] shadow-[0_12px_34px_rgba(15,23,42,.09)] overflow-hidden text-left space-y-4">
          <div className="p-[20px_24px] border-b border-[#E2E8F0] flex items-center justify-between"><div className="flex items-center gap-3"><div className="h-[46px] w-[46px] rounded-[14px] bg-[#0F3A53]/10 text-[#0F3A53] font-extrabold flex items-center justify-center text-base">{initialsOf(booking.therapistName)}</div><div><h3 className="text-[15.5px] font-bold text-[#0F172A]">{booking.therapistName}</h3><p className="text-[12.5px] text-[#64748B] font-medium">{booking.serviceTitle}</p></div></div><span className={`h-6 px-3 rounded-full text-xs font-bold border flex items-center ${booking.manualPayment ? 'bg-[#FFFBEB] text-[#B45309] border-amber-200' : 'bg-[#ECFDF5] text-[#059669] border-[#A7F3D0]'}`}>{booking.manualPayment ? 'AWAITING TRANSFER' : booking.status}</span></div>
