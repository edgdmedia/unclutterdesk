...
describe('decide', () => {
  it('serves a tenant booking subdomain, indexable', () => {
    expect(decide('dr-smith.unclutterdesk.com', config)).toEqual({ kind: 'serve', indexable: true, checkTenant: true });
    expect(decide('demo.unclutterdesk.com', config)).toEqual({ kind: 'serve', indexable: true, checkTenant: true });
  });
  ...
  it('serves a tenant custom domain arriving via Cloudflare for SaaS, indexable', () => {
    expect(decide('booking.drjanetherapy.com', config)).toEqual({ kind: 'serve', indexable: true, checkTenant: true });
  });
  ...
  it('marks the application shell hosts as not indexable', () => {
    expect(decide('app.unclutterdesk.com', config)).toEqual({ kind: 'serve', indexable: false, checkTenant: false });
    expect(decide('admin.unclutterdesk.com', config)).toEqual({ kind: 'serve', indexable: false, checkTenant: false });
  });

  it('does not let a tenant slug that merely contains "app" become non-indexable', () => {
    expect(decide('apple-therapy.unclutterdesk.com', config)).toEqual({ kind: 'serve', indexable: true, checkTenant: true });
    expect(decide('app-therapy.unclutterdesk.com', config)).toEqual({ kind: 'serve', indexable: true, checkTenant: true });
  });

  it('does not look up a practice for the app shell hosts', () => {
    expect(decide('app.unclutterdesk.com', config)).toMatchObject({ checkTenant: false });
    expect(decide('admin.unclutterdesk.com', config)).toMatchObject({ checkTenant: false });
  });

  it('looks up a practice for booking subdomains and custom domains', () => {
    expect(decide('dr-smith.unclutterdesk.com', config)).toMatchObject({ checkTenant: true });
    expect(decide('booking.drjanetherapy.com', config)).toMatchObject({ checkTenant: true });
  });
  ...
  it('does not treat a lookalike domain as our zone', () => {
    ...
    expect(decide('unclutterdesk.com.evil.io', config)).toEqual({ kind: 'serve', indexable: true, checkTenant: true });
    expect(decide('evil-unclutterdesk.com', config)).toEqual({ kind: 'serve', indexable: true, checkTenant: true });
  });
});
