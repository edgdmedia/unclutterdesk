...
8. **Rejected CORS origins return HTTP 500.** *(Fixed: the handler calls `callback(null, false)` instead of throwing, so a disallowed origin simply gets no `Access-Control-Allow-Origin` header.)* The rejection callback throws an unhandled `Error` rather than returning `callback(null, false)`. Noisy, and it surfaces as a server error.

9. **No error monitoring or structured logging.** *(Fixed: Sentry reports 5xx with scrubbed request data, plus a global exception filter now logs every 5xx with a stack and a reference id while returning only a generic body, so an unhandled Prisma error can no longer quote clinical values back to the caller. An external error tracker such as Sentry is still not wired up.)* No Sentry, no pino/winston, only ad-hoc `Logger` calls. Production failures will be discovered by therapists, not by you. `apps/api/src/modules/auth/auth.service.ts:24` also defines an `authDebug` helper that JSON-dumps auth detail into logs — audit what it receives before it is used.
...
18. **Confirm the seeded demo account is absent from production.** *(Confirmed absent — `scripts/preflight.mjs` queried production directly: no demo user, no `smiththerapy.ng` profiles, 1 real tenant.)* `docs/launch-checklist.md` §6 documents `dr.jane@smiththerapy.ng / password123` as a pre-verified login. CI sets `SEED_DB=false`, which is correct, but the seed may have been run manually during setup. Verify directly against the production DB and delete if present. *(Not verifiable from here — live credential testing was out of scope for this audit.)*
...
24. **The `.ics` download was unauthenticated and enumerable — the most serious finding in this audit.** *(Fixed.)* `GET /v1/calendar/bookings/:bookingId/ical` carried no guard at all and loaded the booking by id with no tenant scope. Booking ids are sequential BigInts, so anyone could walk them and retrieve, per booking: the client's name or email, the therapist's name, the service title, the exact appointment time, and **the video room join link**. Jitsi rooms are unauthenticated, so that link alone lets a stranger join a live therapy session. Verified unauthenticated against production; no data was exposed only because no bookings existed yet.
...
27. **Authenticated routes authorised nobody.** *(Fixed.)* Controllers carried `@UseGuards(JwtAuthGuard)` and nothing more, which authenticates identity but authorises nothing — and no role guard existed anywhere in the codebase. 56 routes were reachable by any signed-in account, including a client: reading another client's SOAP notes via `GET /v1/notes/client/:id`, changing practice billing, deactivating a practitioner, editing intake forms.

    This is the limitation of the tenant-isolation work in findings 22-26: those made cross-*practice* access safe, while within a practice there was no authorization at all. `RolesGuard` reads the role from the database because the access token carries none, and `roles.spec.ts` fails the build if an authenticated route omits a role declaration.
